...
PA_MODULE_USAGE(
        _("sink_name=<name for the sink> "
          ...
          "master=<name of sink to apply post process> "
          "rate=<sample rate> "
...
static int sink_process_msg_cb(pa_msgobject *o, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    struct app_userdata *u = PA_SINK(o)->userdata;
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    ...
    AudioPostProcess_Proc(u->mem, n, src, dst);
...
static void sink_input_moving_cb(pa_sink_input *i, pa_sink *dest) {
    ...
    if (u->auto_desc && dest) {
        ...
        pa_proplist_setf(pl, PA_PROP_DEVICE_DESCRIPTION, "Audio Post Process %s on %s",
                         pa_proplist_gets(u->sink->proplist, "device.app.name"), z ? z : dest->name);
...
static pa_hook_result_t palm_policy_set_parameters_cb(pa_palm_policy *pp, pa_palm_policy_set_param_data_t *spd, struct userdata *u) {
    ...
    //  equalizer control
    if (strcmp(ptr, "equalizer") == 0) {
        ptr = strtok(NULL, " ");
        if (strcmp(ptr, "enable") == 0) {
            ptr = strtok(NULL, " ");
            bool enable = (bool) atoi(ptr);
            AudioPostProcess_Equalizer_setEnable(user_data->mem, enable);
        } else if (strcmp(ptr, "param") == 0) {
            ptr = strtok(NULL, " ");
            int preset = atoi(ptr);
            ptr = strtok(NULL, " ");
            int band = atoi(ptr);
            ptr = strtok(NULL, " ");
            float level = atof(ptr);

            if (preset >= 0) {
                AudioPostProcess_Equalizer_setPreset(user_data->mem, preset);
            } else {
...
int pa__init(pa_module *m) {
    ...
    u = pa_xnew0(struct app_userdata, 1);
    u->module = m;
    m->userdata = u;
    ...
    if (!(sink_data.name = pa_xstrdup(pa_modargs_get_value(ma, "sink_name", NULL))))
        sink_data.name = pa_sprintf_malloc("%s.process", master->name);
    ...
    if ((u->auto_desc = !pa_proplist_contains(sink_data.proplist, PA_PROP_DEVICE_DESCRIPTION))) {
        ...
        pa_proplist_setf(sink_data.proplist, PA_PROP_DEVICE_DESCRIPTION, "Audio Post Process %s on %s", sink_data.name, z ? z : master->name);
    }
    ...
    pa_proplist_setf(sink_input_data.proplist, PA_PROP_MEDIA_NAME, "Audio Post Process Stream from %s", pa_proplist_gets(u->sink->proplist, PA_PROP_DEVICE_DESCRIPTION));
    ...
    //  solution memory should be allocated using pa_xnew0
    u->mem = pa_xnew0(AudioPostProcessMemory, 1);
    AudioPostProcess_Init(u->mem, ss.rate, ss.channels);
