...
int AudioProcessingImpl::InitializeLocked(const ProcessingConfig& config) {
  ...
  // }
  if (min_proc_rate > kMaxAECMSampleRateHz) {
    fwd_proc_rate = kMaxAECMSampleRateHz;
  }
...
void AudioProcessingImpl::set_beamformer_auto_aim(bool enable) {
  if (!beamformer_) return;
  beamformer_->SetAutoAim(enable);
}

float AudioProcessingImpl::get_beamformer_direction() {
  if (!beamformer_) return 0;
  return beamformer_->GetDirection();
}
...
bool AudioProcessingImpl::is_data_processed() const {
  ...
  if (enabled_count == 0) {
    ...
    if (level_estimator_->is_enabled()) {
      return false;
    }
...
bool AudioProcessingImpl::output_copy_needed(bool is_data_processed) const {
  ...
  return ((api_format_.output_stream().num_channels() !=
           api_format_.input_stream().num_channels()) || is_data_processed);
}
...
void AudioProcessingImpl::InitializeBeamformer() {
  if (beamformer_enabled_) {
    ...
    beamformer_->Initialize(kChunkSize, split_rate_);
  }
}
