...
const int kLowMeanStartHz = 300;
const int kLowMeanEndHz = 600;
...
const float kCompensationGain = 9.f;
...
NonlinearBeamformer::NonlinearBeamformer(
  ...
  auto_aim_enable_ = true;
}

void NonlinearBeamformer::Initialize(int chunk_size, int sample_rate_hz) {
  chunk_length_ = chunk_size;
  ...
  //  LGE: initial auto aim
  doa_.Initialize(array_geometry_, target_angle_radians_);
}
...
void NonlinearBeamformer::ProcessAudioBlock(const complex_f* const* input,
  ...
  if (auto_aim_enable_)
    AutoAim(input, num_input_channels, num_freq_bins);
...
void NonlinearBeamformer::AutoAim(const complex_f* const* input,
                                  int num_input_channels,
                                  size_t num_freq_bins) {
  if (!doa_.ComputeDoa(input, num_input_channels, num_freq_bins))   //  compute doa and check the direction is updated
    return;

  AimAt(SphericalPointf(doa_.GetDirectionRadians(), 0.f, 1.f));
}
