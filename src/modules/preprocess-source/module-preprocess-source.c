...
typedef enum {
    ...
    PA_ECHO_CANCELLER_ECNR,
} pa_echo_canceller_method_t;

#define DEFAULT_ECHO_CANCELLER "ecnr"
...
struct pa_preprocess_msg {
    pa_msgobject parent;
    bool dead;
    struct userdata *userdata;
};

PA_DEFINE_PRIVATE_CLASS(pa_preprocess_msg, pa_msgobject);
#define PA_PREPROCESS_MSG(o) (pa_preprocess_msg_cast(o))
...
enum {
    SINK_INPUT_MESSAGE_LATENCY_SNAPSHOT = PA_SINK_INPUT_MESSAGE_MAX,
};
...
static void do_push(struct userdata *u) {
    ...
    while (rlen >= u->source_output_blocksize) {
        ...
        lge_preprocess_run(u->ec, rdata, pdata, cdata);
        if (u->save_aec) {
            if (u->canceled_file)
...
static int canceller_process_msg_cb(pa_msgobject *o, int code, void *userdata, int64_t offset, pa_memchunk *chunk) {
    struct pa_preprocess_msg *msg;
    struct userdata *u;

    pa_assert(o);

    msg = PA_PREPROCESS_MSG(o);
     ...
     * asyncmsgq holds a reference to the pa_preprocess_msg object, which
...
static pa_echo_canceller_method_t get_ec_method_from_string(const char *method) {
    if (pa_streq(method, "ecnr"))
        return PA_ECHO_CANCELLER_ECNR;
...
static int init_common(pa_modargs *ma, struct userdata *u, pa_sample_spec *source_ss, pa_channel_map *source_map) {
    ...
    u->ec = pa_xnew0(preprocess_params, 1);
    if (!u->ec) {
    ...
    pa_log("Using AEC engine: %s", ec_string);
...
int pa__init(pa_module*m) {
    ...
    if (!lge_preprocess_init(u->ec, &source_output_ss, &source_output_map, &sink_ss, &sink_map, &source_ss, &source_map, &nframes, pa_modargs_get_value(ma, "aec_args", NULL))) {
    ...
    //    source_data.name = pa_sprintf_malloc("%s.ecnr", source_master->name);
    source_data.name = pa_sprintf_malloc("preprocess-source");
    ...
    if ((u->source_auto_desc = !pa_proplist_contains(source_data.proplist, PA_PROP_DEVICE_DESCRIPTION))) {
        ...
        pa_proplist_setf(source_data.proplist, PA_PROP_DEVICE_DESCRIPTION, "%s (preprocessed with %s)",
    ...
    if ((u->sink_auto_desc = !pa_proplist_contains(sink_data.proplist, PA_PROP_DEVICE_DESCRIPTION))) {
        ...
        pa_proplist_setf(sink_data.proplist, PA_PROP_DEVICE_DESCRIPTION, "%s (preprocessed with %s)",
    ...
    pa_proplist_sets(source_output_data.proplist, PA_PROP_MEDIA_NAME, "preprocess Stream");
    ...
    pa_proplist_sets(sink_input_data.proplist, PA_PROP_MEDIA_NAME, "preprocess Stream");
    ...
    u->source_memblockq = pa_memblockq_new("module-preprocess source_memblockq", 0, MEMBLOCKQ_MAXLENGTH, 0,
        &source_output_ss, 1, 1, 0, &silence);
    u->sink_memblockq = pa_memblockq_new("module-preprocess sink_memblockq", 0, MEMBLOCKQ_MAXLENGTH, 0,
    ...
    u->msg = pa_msgobject_new(pa_preprocess_msg);
    u->msg->parent.process_msg = canceller_process_msg_cb;
    u->msg->userdata = u;
...
int main(int argc, char* argv[]) {
    ...
    u.ec->done(u.ec);
    u.msg->dead = true;
    pa_echo_canceller_msg_unref(u.msg);
